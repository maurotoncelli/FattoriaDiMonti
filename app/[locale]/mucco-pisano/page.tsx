                    ...
                    <div className="mt-8 mb-24 sm:mt-12 sm:mb-32 w-full">
                        <div className="mx-auto grid max-w-[1200px] grid-cols-1 gap-16 md:grid-cols-2 lg:gap-24 relative z-10 w-full">
                            {muccoPisanoData.essiccata.products?.map((product) => (
                                <div 
                                    key={product.id}
                                    className="group relative flex flex-col items-center text-center fade-up-text"
                                >
                                    {/* Foto mockup packaging (provvisorie — Giacomo) */}
                                    <div className="relative mb-6 flex h-[350px] sm:h-[400px] w-full items-end justify-center">
                                        <div className="absolute bottom-[-15px] h-6 w-56 rounded-full bg-[#3D332A]/20 blur-xl" />
                                        <div className="relative h-full w-[280px] sm:w-[320px]">
                                            {product.image?.src ? (
                                                <Image 
                                                    ...
                                                    className="object-contain drop-shadow-[0_15px_25px_rgba(40,30,20,0.25)]" 
                                                />
                                            ) : (
                                    ...
                                    {/* Product Text & Tags */}
                                    <span className="mb-3 font-inter text-[10px] uppercase tracking-[0.24em] text-[var(--olive)]/80 block w-full text-center">
                                        {product.subtitle}
                                    </span>
                                    <h3 className="font-playfair text-4xl italic leading-none text-[var(--mucco-pisano)] block w-full text-center">
                                        {product.name}
                                    </h3>
                                    <p className="mt-5 min-h-[80px] max-w-sm text-sm md:text-base leading-[1.8] text-[var(--mucco-pisano)]/75 mx-auto text-center">
                                        {product.description}
                                    </p>
                                    <div className="mt-6 flex flex-wrap justify-center gap-2 max-w-[80%] mx-auto">
                                        {product.tags?.map((tag: string) => (
                                            <span
                                                key={tag}
                                                className="rounded-full border border-[var(--mucco-pisano)]/15 bg-transparent px-3 py-1 font-inter text-[9px] uppercase tracking-[0.16em] text-[var(--mucco-pisano)]/70"
                                            >
                                                {tag}
                                            </span>
                                        ))}
                        ...
                        {/* CTA unica — richiedi maggiori informazioni */}
                        <div className="mt-20 flex justify-center w-full fade-up-text">
                            <button
                                onClick={() => setConciergeOpen(true, 'carne-secca')}
                            ...
                            >
                                ...
                                <span className="relative z-10 font-inter text-xs font-semibold uppercase tracking-[0.2em] text-[var(--mucco-pisano)] transition-colors duration-500 group-hover:text-[#F3EFE7]">
                                    {muccoPisanoData.essiccata.ctaLabel}
                                </span>
                            </button>
                        </div>
                    </div>
